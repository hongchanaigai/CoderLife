/**********************************************************
 * Author         : huang
 * Creat modified : 2020-07-26 21:05
 * Last modified  : 2020-07-26 21:05
 * Filename       : 02_定义和声明的区别.c
 * Description    : 实现strlen函数功能,不用库函数strlen()
 * *******************************************************/

#include <stdio.h>

//1. 编译器很笨,在main函数调用其他函数,只会往前找函数的定义
//2. 如果找不到函数的定义,找函数的声明,如果没有声明,C编译器会警告,C++会出错
//3. 如果没有定义,直接调用一个不存在的函数,会报错
//4. 如果定义没有放在main函数的前面,那么在调用函数前需要声明,声明加不加extern是一样的
