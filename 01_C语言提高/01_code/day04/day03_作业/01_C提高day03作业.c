1. 	char buf[] = "abcdef";
	...
	//下面有啥区别？
	const char *p = buf;	//const修饰*,指针指向的内存不能修改
	char const *p = buf;	//const修饰*,指针指向的内存不能修改
	char * const p = buf;	//const修饰p,指针变量本身的值不能修改
	const char *const *p = buf;	//const修饰*,指针变量指向的内存不能修改
...
2.	char *p1[] = {"1111", "2222", "3333"};
	...
	sizeof(p2)/sizeof(p2[0]) = ?	//(4 * 10) / 4 = 10
	
	char p3[][30] = {"1111", "2222", "3333"};
	sizeof(p3)/sizeof(p3[0]) = ?	//(1 * 3 * 30) / (1 * 1 * 30) = 3
	
	char p4[10][30] = {"1111", "2222", "3333"};
	sizeof(p4)/sizeof(p4[0]) = ?	//(1 * 10 * 30) / (1 * 1 * 30) = 10
